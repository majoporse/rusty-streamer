...
/// struct for typed errors of method [`get_actor_by_id`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetActorByIdError {
    Status500(),
    UnknownValue(serde_json::Value),
}
...
pub async fn create_actor(configuration: &configuration::Configuration, new_actor: models::NewActor) -> Result<models::Actor, Error<CreateActorError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Actor`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Actor`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_all_actors(configuration: &configuration::Configuration, limit: i64, offset: i64) -> Result<Vec<models::Actor>, Error<GetAllActorsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetAllActorsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
